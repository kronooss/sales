...
import { Package } from "@dashboard/components/Package";
...
const OrderDetailsPage: React.FC<OrderDetailsPageProps> = props => {
  ...
  return (
    ...
    >
      ...
      useEffect(() => {
        if (!data[order?.id] || !data[order?.id]?.metadata || !data[order?.id]?.metadata.length) return;
        const packagesValue = data[order?.id]?.metadata.find(item => item.key === 'packages').value;
        const validJSONString = packagesValue.replace(/'/g, '"');
        setPackages(JSON.parse(validJSONString));
        
        const packageType = data[order?.id]?.metadata.find(item => item.key === 'packagingType').value;
          ...
          <DetailPageLayout>
            ...
            <DetailPageLayout.Content data-test-id="order-fulfillment">
              ...
              {/* <Package
                ...
                onChange={x => handleChangeMetadata(x, order?.id)}
              /> */}
              {order?.shippingMethod.id === "U2hpcHBpbmdNZXRob2Q6MTU=" ? (
                <Package
                  packages={packages}
                  services={services}
